...
--  ----------Borda Count Voting System
    ...
    let cndCount = candidatesCount election
    ...
    putStr "Point For:\n" >> putStr "First Position:  " >> putStrLn (show cndCount) >> putStr "Second Position: " >> putStrLn (show $ cndCount -1) >> putStr "Third Position:  " >> putStrLn (show $ cndCount -2) >> putStrLn "....\n"

    printPosPoints (pairPosPoints election) 1
...
{--Drop first preference--}
dropFirst :: Poll -> Poll
dropFirst p = cleanEmpty[tail x | x <- p]


{--First list contains the candidate and second the times it appears at first position. 
Every next sublist in the two lists holds information for the (tail p) e.g the Poll with updated/removed first preferences.--}
countNexPos :: Poll -> [Poll]
countNexPos p = [(map (\z -> [ head x | x <- z]) d), (map (\z -> [ length  x | x <- z]) d)]
              --(e,f)
    where
        c = take (candidatesCount p) $ iterate dropFirst p -- drop until no more first preferences
        d = [ group $ sort $ firstPreference  x | x <- c] -- group and sort each of the reduced votes
        -- e = map (\z -> [ head x | x <- z]) d -- list with candidates 
        -- f = map (\z -> [ length  x | x <- z]) d -- how many times the candidate appears in first position of the reduced list



{--Show how many points each candidate has according to the position it takes in the vote.--}
pairPosPoints :: Poll -> [[[Char]]]
pairPosPoints p = map (\z -> [ "candidate " ++ show x ++ ": " ++ show y ++ " times" | (x,y) <- z]) [zip x y | (x,y) <- (zip (head b) (head $ tail b))]
                -- g
    where
        b = countNexPos p -- two lists first holding candidates and second number of times per position
        -- c = head b -- list with list of candidates (from each removal of first firstPreference) 
        -- d = head $ tail b -- list with number of times per this position
        -- e = zip c d  -- pair 
        -- f =[ zip x y | (x,y) <- e] -- pair each candidate with occurrences at given position
        -- g = map (\z -> [ "candidate " ++ show x ++ ": " ++ show y ++ " times" | (x,y) <- z]) f  -- prepare for output


{--Top funciton for outputting the results --}
printPosPoints :: [[[Char]]] -> Int -> IO()
printPosPoints (x:xs) n = do
    putStr "Candidates appear at: " >> putStr(show n) >> putStrLn " position"
    mapM_ putStrLn x
    putStr "\n"
    printPosPoints xs (n+1) 
printPosPoints [] _ = putStr " "
